...
function saveAddress(event) {
    ...
    // Get or initialize user data in localStorage
    let userData = JSON.parse(localStorage.getItem('user')) || {
        email: user.email,
        addresses: []
    };
    ...
    if (isEditing) {
        ...
        const index = userData.addresses.findIndex(addr => addr.id === isEditing);
        if (index !== -1) {
...
function deleteAddress(addressId) {
    ...
    if (userData.addresses.length > 0 && !userData.addresses.some(addr => addr.isDefault)) {
        ...
        localStorage.setItem('defaultAddress', JSON.stringify(userData.addresses[0]));
    }
