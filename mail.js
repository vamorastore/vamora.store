...
async function loadOrders() {
  try {
    ...
    allOrders = querySnapshot.docs.map(doc => {
      ...
      if (data.isNewOrder) {
        sendNewOrderNotification(doc.id, data);
        // Mark as not new anymore
        db.collection("orders").doc(doc.id).update({ isNewOrder: false });
      }
...
async function sendNewOrderNotification(orderId, orderData) {
  const scriptUrl = "https://script.google.com/macros/s/AKfycbwcFrAvD11rprlliEbAqBS82we4u-FQozVBKOgQkqliTxDTbn5keEP7jIs8w9fSMXFcCw/exec"; // Replace with your actual URL
  ...
  try {
    const response = await fetch(scriptUrl, {
      ...
      body: JSON.stringify({
        orderId: orderId,
        orderTime: orderData.timestamp?.toDate().toLocaleString() || new Date().toLocaleString(),
        customerName: orderData.shippingAddress?.name || "N/A",
...
function renderOrders(page) {
  ...
  if (ordersToDisplay.length === 0) {
    ordersTableBody.innerHTML = `<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">No orders found</td></tr>`;
    return;
  }
  ...
  ordersToDisplay.forEach(order => {
    ...
    orderRow.innerHTML = `
      ...
      <td class="px-4 py-3 whitespace-nowrap"><span class="status-badge status-${order.status}">${order.status}</span></td>
...
async function showOrderDetails(orderId) {
  ...
  document.getElementById('courierServiceSelect').value = order.courierService || "";
  ...
  order.items.forEach(item => {
    ...
    itemElement.innerHTML = `
      <img src="${item.image || 'https://via.placeholder.com/60'}" alt="${item.name}" class="w-16 h-16 object-cover rounded mr-3">
      <div class="flex-1">
...
function setupPagination() {
  ...
  paginationInfo.innerHTML = `Showing <span class="font-medium">${startOrder}</span> to <span class="font-medium">${endOrder}</span> of <span class="font-medium">${totalOrders}</span> results`;

  for (let i = 1; i <= totalPages; i++) {
    const pageBtn = document.createElement('button');
    pageBtn.className = `relative inline-flex items-center px-4 py-2 border text-sm font-medium ${i === currentPage ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`;
        ...
        html += `
          <article class="submission-card" data-status="${data.status || 'new'}" data-name="${escapeHtml(data.name)}" data-email="${escapeHtml(data.email)}" data-message="${escapeHtml(data.message)}">
            ...
            <div class="submission-actions">
              ${data.status !== 'read' ? `<button class="btn mark-read" data-id="${doc.id}" type="button" aria-label="Mark submission as read"><i class="fas fa-check"></i> Mark as Read</button>` : ''}
              <button class="btn delete" data-id="${doc.id}" type="button" aria-label="Delete submission"><i class="fas fa-trash"></i> Delete</button>
            </div>
          </article>
        `;
      });
...
function showToast(message, type = 'success') {
  ...
  toast.className = `hidden fixed bottom-4 right-4 px-4 py-2 rounded-md shadow-lg text-white ${type === 'success' ? 'bg-green-500' : 'bg-red-500'}`;
