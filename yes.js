...
function renderOrders(orders) {
    ...
    ordersContainer.innerHTML = orders.map(order => `
        <div class="order-item border-b border-gray-200 py-6 px-4 rounded-lg mb-4 bg-white shadow-sm">
            <!-- Order Info - Same for both desktop and mobile -->
            ...
            <!-- Order Status - Different layout for mobile -->
            <div class="mb-6">
                <!-- Mobile View - Vertical Status -->
                <div class="md:hidden">
            ...
            <div class="mt-4">
                ...
                ${order.items.map(item => `
                    <div class="flex items-start md:items-center mt-2 p-2 bg-gray-50 rounded">
                        <img src="${item.image || 'https://via.placeholder.com/50'}" 
                             alt="${item.title || 'Item'}" 
                             class="w-12 h-12 rounded mr-3 object-cover">
                        <div class="flex-1">
                            <p class="font-medium text-sm md:text-base">${item.title}</p>
                            <div class="grid grid-cols-2 md:flex md:justify-between text-xs md:text-sm text-gray-500 mt-1 md:mt-0">
                                <span class="mb-1 md:mb-0">Size: ${item.size || '-'}</span>
                                <span class="mb-1 md:mb-0">Qty: ${item.quantity || 1}</span>
                                <span class="col-span-2 md:col-span-1">${item.price || '₹0.00'}</span>
                            </div>
                        </div>
                    </div>
                `).join('')}
