...
function showAccountInfo(event) {
    ...
    if (loggedInUser && loggedInUser.email) {
        ...
        // This is the key line - sets the readonly email field value
        document.getElementById('emailDisplay').value = loggedInUser.email || '';
...
function loadAccountInfo(email) {
    ...
    if (loggedInUser) {
        ...
        // Set checkout email field and make it readonly if user is logged in
        if (loggedInUser.email) {
        ...
        // Reset checkout email field if not logged in
...
function resetForms() {
    ...
    // Reset the email field readonly state
    const loginEmailField = document.getElementById('login-email');
    if (loginEmailField) {
        loginEmailField.readOnly = false;
        loginEmailField.classList.remove('bg-gray-100');
    }
}
...
function logoutUser(event) {
    ...
    document.getElementById('displayName').textContent = '';
    document.getElementById('displayEmail').textContent = '';
    document.getElementById('emailDisplay').value = '';  // Clear the readonly email field
...
function updateLoginButton() {
    ...
    if (loginEmailField) {
        if (user && user.email) {
            loginEmailField.value = user.email;
            loginEmailField.readOnly = true;
            loginEmailField.classList.add('bg-gray-100'); // Visual indication it's readonly
        } else if (rememberedUser) {
            loginEmailField.value = rememberedUser.email;
            loginEmailField.readOnly = true;
            loginEmailField.classList.add('bg-gray-100');
        } else {
            loginEmailField.readOnly = false;
            loginEmailField.classList.remove('bg-gray-100');
        }
...
function handlePaymentFailure(response, orderId) {
    ...
    alert(`Payment failed for order ${orderId}. Please try again.`);
    
    // You can log this failure for analytics
...
function loadRazorpayScript(callback) {
    ...
    script.onerror = function() {
        ...
        document.getElementById('paymentFailedPopup').classList.add('active');
    };
...
// This function handles GitHub Pages specific issues
function handleGitHubPagesCompatibility() {
    ...
    // Ensure Razorpay can open popups
    window.RazorpayConfig = {
        integration: 'github_pages',
        popup: true
    };
}
...
// Run when page loads
window.addEventListener('load', function() {
    handleGitHubPagesCompatibility();
    
    // If you need to detect mobile for QR code preference
    if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        document.querySelector('.payment-method[data-method="qr_code"]').style.display = 'flex';
    }
});
...
function placeOrder() {
    ...
    // 3. Razorpay options with GitHub Pages specific fixes
    const options = {
        key: "rzp_live_DPartLBDccSG34", // Your Razorpay key
        amount: amount,
        currency: "INR",
        name: "VAMORA.STORE",
        ...
        method: {
            ...
            wallet: true,
            upi: true,
            qr_code: true // Enable QR code payments
        },
        modal: {
            ondismiss: function() {
    ...
    // 5. GitHub Pages specific fixes
    rzp.on('payment.loaded', function(response) {
        // Fix iframe sandboxing issues
        const iframes = document.querySelectorAll('iframe[src*="razorpay.com"]');
        iframes.forEach(iframe => {
            iframe.setAttribute('sandbox', 'allow-scripts allow-same-origin allow-forms');
        });
        
        // Ensure QR code is visible
        const qrContainers = document.querySelectorAll('.razorpay-qr-code-container');
        qrContainers.forEach(container => {
            container.style.display = 'block';
            container.style.visibility = 'visible';
        });
        
        // Force HTTPS for QR code images
        const images = document.querySelectorAll('.razorpay-qr-code-container img');
        images.forEach(img => {
    ...
    // 6. Additional timeout to ensure QR code appears
    setTimeout(() => {
        const qrContainers = document.querySelectorAll('.razorpay-qr-code-container');
        qrContainers.forEach(container => {
            container.style.display = 'block';
...
// Load cart and render order summary when page loads
window.addEventListener('load', function() {
