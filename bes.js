    ...
    if (!addresses || addresses.length === 0) {
        addressContainer.innerHTML = `
            <div class="text-center text-gray-500 py-8">
                ...
                <button onclick="showAddAddressModal()" class="mt-4 px-4 py-2 bg-black text-white rounded hover:bg-gray-800">
                    Add Address
                </button>
            </div>
        `;
    ...
    addressContainer.innerHTML = `
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ${addresses.map(address => `
                <div class="address-card ${address.isDefault ? 'border-2 border-blue-500' : 'border border-gray-200'} bg-white p-4 rounded-lg shadow-sm">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            <h3 class="font-medium text-lg">${address.fullName}</h3>
                            ${address.isDefault ? 
                                '<span class="inline-block ml-2 px-2 py-0.5 text-xs bg-blue-100 text-blue-800 rounded">Default</span>' : ''}
                        </div>
                    ...
                    <div class="space-y-1 text-sm text-gray-600">
                        <p><i class="fas fa-phone-alt mr-2"></i> ${address.phoneNumber}</p>
                        <p><i class="fas fa-map-marker-alt mr-2"></i> ${address.addressLine1}</p>
                        ${address.addressLine2 ? `<p class="ml-6">${address.addressLine2}</p>` : ''}
                        <p class="ml-6">${address.city}, ${address.state} ${address.postalCode}</p>
                        <p class="ml-6">${address.country}</p>
                    </div>
                    
                    <div class="mt-3 pt-2 border-t border-gray-100">
                        <span class="inline-block px-2 py-1 text-xs rounded-full 
                            ${address.addressType === 'home' ? 'bg-green-100 text-green-800' : 
                              address.addressType === 'work' ? 'bg-blue-100 text-blue-800' : 
        ...
        <div class="mt-6 text-center">
            <button onclick="showAddAddressModal()" class="px-4 py-2 bg-black text-white rounded hover:bg-gray-800">
...
async function saveAddress(event) {
    ...
    try {
        ...
        // Validate fields
        const requiredFields = ['fullName', 'phoneNumber', 'addressLine1', 'city', 'state', 'postalCode', 'country'];
        const errors = requiredFields.filter(field => !address[field]);
