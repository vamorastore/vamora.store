...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addAddressLink) {
        addAddressLink.addEventListener('click', function(e) {
            e.preventDefault();
            showAddAddressModal(e);
        });
    }
});
// Add this function to validate the address form
function validateAddressForm(address) {
    ...
    if (!address.postalCode || !/^\d{6}$/.test(address.postalCode)) {
        document.getElementById('postalCodeError').textContent = 'Valid 6-digit postal code is required';
        isValid = false;
    }
